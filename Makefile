# TODO fix issue with not loading .envrc
# include .envrc
...
.PHONY: run/api
run/api:
	@bash -c 'source .env "JWT_SECRET: $$JWT_SECRET" && go run ./cmd/api -db-dsn=$$GREENLIGHT_DB_DSN -jwt-secret=$$JWT_SECRET'
...
production_host_ip = '159.65.88.242'
